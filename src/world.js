...
export class World {
  ...
  _buildSegments() {
    ...
    const deepMat = new THREE.MeshStandardMaterial({ color: 0x21252f, roughness: 0.92, metalness: 0.12 });  // outer colonnade / vaulting (a touch darker so it recedes)
    ...
    for (let i = 0; i < COUNT; i++) {
      ...
      for (const sx of [-1, 1]) {
        ...
        // --- OUTER COLONNADE: a taller pillar set further back. Because it is
        //     farther from the track it parallaxes against the near columns,
        //     giving the corridor real cathedral depth and height.
        const outer = new THREE.Mesh(new THREE.CylinderGeometry(0.62, 0.82, 12, 7), deepMat);
        outer.position.set(sx * (HALL_W / 2 + 3.6), 6, -SEG * 0.12);
        seg.add(outer);
        const cap = new THREE.Mesh(new THREE.BoxGeometry(1.7, 0.5, 1.7), deepMat);
        cap.position.set(sx * (HALL_W / 2 + 3.6), 12, -SEG * 0.12); seg.add(cap);

        // --- FLYING BUTTRESS: a diagonal strut leaning from the outer pillar in
        //     to the near pillar's top — classic gothic support, high overhead.
        {
          const p1x = sx * (HALL_W / 2), p1y = 6.4;              // near pillar top
          const p2x = sx * (HALL_W / 2 + 3.4), p2y = 4.4;        // outer pillar upper
          const dx = p2x - p1x, dy = p2y - p1y, len = Math.hypot(dx, dy);
          const strut = new THREE.Mesh(new THREE.BoxGeometry(0.26, len, 0.26), deepMat);
          strut.position.set((p1x + p2x) / 2, (p1y + p2y) / 2, -SEG * 0.12);
          strut.rotation.z = Math.atan2(-dx, dy);
          seg.add(strut);
        }

        // --- rubble at the curb base — small broken masonry for ground detail
        const rock = new THREE.Mesh(new THREE.IcosahedronGeometry(0.16 + Math.random() * 0.16, 0), stoneMat);
        rock.position.set(sx * (HALL_W / 2 - 0.05 + Math.random() * 0.5), 0.08, (Math.random() - 0.5) * SEG * 0.8);
        rock.rotation.set(Math.random() * 3, Math.random() * 3, Math.random() * 3);
        seg.add(rock);
      }

      // --- POINTED VAULT: two ribs spring from the near-pillar tops to a boss
      //     high overhead (apex ~8.4), reading as a nave ceiling. Kept well above
      //     the play field so it never crosses the road. Every other segment.
      if (i % 2 === 0) {
        const apex = 8.4;
        for (const sx of [-1, 1]) {
          const p1x = sx * (HALL_W / 2), p1y = 6.4, p2x = 0, p2y = apex;
          const dx = p2x - p1x, dy = p2y - p1y, len = Math.hypot(dx, dy);
          const rib = new THREE.Mesh(new THREE.BoxGeometry(0.22, len, 0.3), deepMat);
          rib.position.set((p1x + p2x) / 2, (p1y + p2y) / 2, 0);
          rib.rotation.z = Math.atan2(-dx, dy);
          seg.add(rib);
        }
        const boss = new THREE.Mesh(new THREE.SphereGeometry(0.3, 8, 6), deepMat);
        boss.position.set(0, apex, 0); seg.add(boss);
      }
  ...
  _buildFarScenery() {
    // A ruined city on the far horizon — huge cathedral towers and spires that
    // scroll SLOWER than the track (parallax), so the world reads as genuinely
    // deep. Unlit silhouettes + a few lit windows so they read against any sky.
    const grp = new THREE.Group();
    this._farItems = [];
    this._farSpan = 5; this._farGap = 66;
    const silhouette = new THREE.MeshBasicMaterial({ color: 0x0c0e18, fog: false });
    const winMat = new THREE.MeshBasicMaterial({ color: 0xffb44a, fog: false, transparent: true, opacity: 0.82 });
    for (let i = 0; i < this._farSpan; i++) {
      for (const sx of [-1, 1]) {
        const c = new THREE.Group();
        const h = 28 + Math.random() * 42;
        const body = new THREE.Mesh(new THREE.CylinderGeometry(3.4, 6, h, 5), silhouette);
        body.position.y = h / 2; c.add(body);
        const spire = new THREE.Mesh(new THREE.ConeGeometry(3.5, 14 + Math.random() * 14, 5), silhouette);
        spire.position.y = h + 7; c.add(spire);
        // a few lit windows climbing the tower
        for (let w = 0; w < 4; w++) {
          const win = new THREE.Mesh(new THREE.PlaneGeometry(0.7, 1.3), winMat);
          win.position.set((Math.random() - 0.5) * 5, 7 + Math.random() * (h - 10), 6.02);
          c.add(win);
        }
        c.position.set(sx * (44 + Math.random() * 48), -3, -52 - i * this._farGap);
        c.rotation.y = (Math.random() - 0.5) * 0.4;
        grp.add(c); this._farItems.push(c);
      }
    }
    this.farScenery = grp;
    this.scene.add(grp);
  ...
  update(dt) {
    ...
    // parallax: the distant city drifts by at a fraction of the track speed, so
    // the near columns rush past while the skyline barely moves — real depth.
    if (this._farItems) {
      const recycle = this._farSpan * this._farGap;
      const camZ = this.camera.position.z;
      for (const c of this._farItems) {
        c.position.z += this.speed * dt * 0.4;
        if (c.position.z > camZ + 30) c.position.z -= recycle;
      }
    }
